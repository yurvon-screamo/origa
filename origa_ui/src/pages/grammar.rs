    ...
    view! {
        <AppLayout active_tab="grammar".to_string()>
            ...
            <div class="section">
                <div class="section-header">
                    <div>
                        ...
                        <p class="section-subtitle">
                            {move || {
                                let level = selected_level();
                                let count = filtered_grammar().len();
                                format!("{} конструкций уровня {}", count, level.to_string())
                            }}
                        </p>
                    </div>
                </div>
                ...
                // Empty state
                <Show
                    when=move || filtered_grammar().is_empty()
                    fallback=|| view! { <div></div> }
                >
                    <div class="empty-state">
                        <div class="empty-icon">📝</div>
                        <h3 class="empty-title">Грамматические конструкции не найдены</h3>
                        <p class="empty-description">
                            {move || if search_query().is_empty() {
                                format!("В уровне {} пока нет конструкций", selected_level().to_string())
                            } else {
...
fn create_mocks() -> Vec<GrammarCardData> {
    vec![
        ...
        GrammarCardData {
            ...
            attachment_rules: "Глагол в отрицательной форме (未然形) + なければ + ならない".to_string(),
            difficulty: 60,
