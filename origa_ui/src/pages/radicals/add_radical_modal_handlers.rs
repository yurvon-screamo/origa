...
use origa::use_cases::CreateRadicalCardUseCase;
...
impl ModalHandlers {
    pub fn new(state: &ModalState, is_open: RwSignal<bool>) -> Self {
        ...
        let on_add = {
            let state = state.clone();
            Callback::new(move |_| {
                let radical_list: Vec<char> = state.selected_radicals.get().into_iter().collect();
                ...
                spawn_local(async move {
                    let use_case = CreateRadicalCardUseCase::new(&repository);
                    match use_case.execute(radical_list).await {
                        Ok(_) => {
