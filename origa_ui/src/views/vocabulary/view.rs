...
use crate::views::vocabulary::create::CreateModal;
...
fn process_cards_data(cards: &Vec<StudyCard>) -> ProcessedCardsData {
    let vocabulary_cards: Vec<StudyCard> = cards
        .iter()
        .filter(|c| matches!(c.card(), Card::Vocabulary(_)))
        .cloned()
        .collect();

    let mapped_cards = vocabulary_cards
        .iter()
