...
Добивание [GhostState] — параллельная FSRS механика закрепления дважды подряд проваленной карточки (successive relearning, `docs/plans/ghost-relearning.md`): лестница распределённых показов до трёх успехов подряд. Value object рядом с `MemoryState`: `Active { rung, due_at, last_transition_at }` либо терминальный маркер `Resolved`/`Expired` (маркеры участвуют в LWW-мерже). FSRS-планировщик добиванием не модифицируется. В коде: `domain::memory::GhostState`.

Ступень [GhostRung] — шаг лестницы добивания: First (12ч) / Second (1д) / Third (3д). Позиция ступени кодирует накопленные успехи: First = 0, Second = 1, Third = 2; успех на Third — третий подряд — закрывает добивание. Отдельного поля «успехи» нет: ступень — единственный источник истины. В коде: `domain::memory::GhostRung`.

Окно [ghost window] — момент `due_at` текущей ступени: шаг добивания засчитывается только при явном показе после открытия окна (`due_at <= now`); показ вне окна двигает только FSRS. В коде: `GhostState::Active.due_at`.

Счётчик подряд-провалов [consecutive_again] — предспавновый счётчик подряд идущих `Again`-рейтингов карточки; два подряд (на не-новой карте) порождают добивание. Сбрасывается любым `Good|Easy`, спавном и входом в терминальное состояние. Мержится по `max()`. В коде: поле `MemoryHistory` `consecutive_again`.

Спавн [ghost spawn] — появление активного добивания: `consecutive_again >= 2 && карта не новая && ghost != Active`. Терминальные состояния замещаются: новая серия провалов легитимно начинает новый цикл.

Списание [GhostState::Expired] — терминальное состояние добивания после 30 дней без единого шага (TTL от `last_transition_at`, ленивая проверка при отборе и рейтинге). Проходит без последствий для FSRS-состояния карточки. В коде: вариант `GhostState::Expired`.

Рука добиваний [GhostHand] — высший приоритет ядра урока после избранного: карты с активным добиванием и открытым окном, наиболее просроченные первыми, без отдельного дневного лимита (общий бюджет `MAX_LESSON_SIZE`). Единственный канал показа карты с активным добиванием (core-отбор и padding её исключают; избранное — исключение-дедуп). В UI невидима. В коде: `lesson_builder::selection::collect_ghost_hand`.

Контекст рейтинга [RatingContext] — происхождение рейтинга: `Explicit` (явный показ карточки пользователю — двигает добивание) либо `Implicit` (неявный: двойной рейтинг грамматики, «уже знаю», сидирование знакомства — добивания не трогает). В коде: `domain::srs::RatingContext`.

Отметка «знаю» [marked_known_at] — таймстемп последнего нажатия «Знаю» на карточке (`MarkCardAsKnownUseCase`). Не путать с «уже знаю» (контекст рейтинга `RatingContext::Implicit`): то — происхождение рейтинга, это — событие явного действия пользователя. Карта с отметкой текущих локальных суток (как дневные лимиты) молчит в канале компаньонов до конца дня; назавтра канал снова открыт. На известной карте повторное «Знаю» обновляет только отметку (память нетронута). В коде: `MemoryHistory::marked_known_at`, предикат `marked_known_today`.

---

## Карточки

### Счётный суффикс (counter)

Тип карточки: один счётный суффикс (本) со всеми его связками — одна карта
(issue #415). Семантика («что считают») — память самой карты в
`StandardLesson`; связки — свои `MemoryHistory` в `CounterReview`.

### Связка (binding)

Ячейка «число × чтение» счётного суффикса (3×本 → さんぼん): собственная
FSRS-память, рейтинг мимо `rate_card`. Числа 1–10, 0 = вопросительное 何;
лексикализованные исключения >10 (日 14/20/24, 歳 20).

### Композитный слот (counter bindings session)

Один слот урока: пачка мини-вопросов связок подряд; завершение —
агрегатный рейтинг семантике (Good / любая ошибка → Again) штатным
рейтинговым путём.
