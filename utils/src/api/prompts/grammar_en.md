...
<prompt>
  <task>
    ...
    Your task: create a detailed description of a Japanese grammar rule
    in English for a JLPT student.
  </task>
  ...
  <success_brief>
    ...
    <quality_criteria>
      <criterion name="language">
        ENGLISH TEXT ONLY in explanations.
        Japanese only in examples, patterns and tables
      </criterion>
      <criterion name="audience">
        English-speaking JLPT {level}
      </criterion>
      <criterion name="title">
        title must be the grammar pattern itself (e.g. ～ます, ～てください)
      </criterion>
      <criterion name="brevity">
        short_description: 3-6 words
      </criterion>
      <criterion name="examples">
        4-6 examples with translation in code blocks
      </criterion>
  ...
  <rules>
    ...
    <rule id="4">
      Format examples in code blocks: Japanese sentence, then translation
    </rule>
