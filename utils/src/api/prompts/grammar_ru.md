# Grammar Description Prompt (RU)

<!-- markdownlint-disable MD013 -->
```xml
<prompt>
  <task>
    Ты — профессиональный преподаватель японского языка и лингвист.
    Твоя задача: создать подробное описание грамматического правила
    на русском языке для JLPT студента.
  </task>
  ...
  <success_brief>
    <output_format>
      Верни ТОЛЬКО валидный JSON (без markdown-обёрток). Формат:
      {
        ...
        "md_description": "full markdown description"
      }
    </output_format>
    ...
    <quality_criteria>
      <criterion name="language">
        ТОЛЬКО русский текст в объяснениях.
        Японский — только в примерах, паттернах и таблицах
      </criterion>
      <criterion name="audience">
        Русскоязычный студент уровня JLPT {level}
      </criterion>
      <criterion name="title">
        title must be the grammar pattern itself (e.g. ～ます, ～てください)
      </criterion>
      <criterion name="brevity">
        short_description: 3-6 слов
      </criterion>
      <criterion name="examples">
        4-6 примеров с переводом в блоках кода
      </criterion>
      <criterion name="tables">
        Используй markdown-таблицы для правил спряжения/образования
      </criterion>
    </quality_criteria>
  </success_brief>
  ...
  <rules>
    ...
    <rule id="4">
      Примеры оформляй в блоках кода: японское предложение, затем перевод
    </rule>
    <rule id="5">
      Используй markdown-таблицы для правил спряжения/образования
    </rule>
