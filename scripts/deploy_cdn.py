...
VERSIONED_FILES: list[str] = [
    ...
    "grammar/grammar_precompute_v3.rkyv",
...
def validate_grammar_if_changed(changed: list[str]) -> None:
    ...
    """
    grammar_targets = [
        p for p in ("grammar/grammar_v2.json", "grammar/grammar_v3.json") if p in changed
    ]
    ...
    for target in grammar_targets:
        ...
        if result.returncode != 0:
            print(
                f"  {target} FAILED validation — deploy aborted",
                file=sys.stderr,
            )
            sys.exit(1)
