"""
Phrase translation validation script using LLM API.

Validates ML-generated translations (Russian and English) for phrases from the CDN dataset.
Detects ML garbage: hallucinations, literal name translations, Chinese readings,
and complete nonsense in either translation.

Features:
- Checkpoint every batch (auto-resume by ID on restart)
- Graceful shutdown on Ctrl+C
- Pre-filter: skips LLM call when both translations are empty

Usage:
    python scripts/validate_phrase_translations.py --api-key <KEY> --input cdn/phrases/data
    python scripts/validate_phrase_translations.py --api-key <KEY> --input cdn/phrases/data --output report.json
...
TRANSLATION_VALIDATION_PROMPT = """You validate Japanese phrase translations for a language learning app.
The translations were generated by ML models and may contain errors.

Y = keep, N = remove.

## Y (keep — acceptable translations)
- Slightly inaccurate but conveys the general meaning
- Unnatural phrasing but factually correct
- Approximate translation of fillers/interjections (e.g. "えっと" → "umm")
- Minor tone/register mismatch

## N (remove — ML garbage in EITHER Russian OR English)
1. HALLUCINATION: Translation adds facts or words not present in the original Japanese.
   JP: "普通の焼酎しかない" → RU: "обычная печь" (焼酎=shochu alcohol, NOT oven)
   JP: "ピンチです！" → RU: "пингвинец!" (ピンチ=pinch/crisis, NOT penguin)

2. LITERAL NAME TRANSLATION: Names translated by kanji meaning instead of reading.
   白河 → "White River" / "Байхэ" (should be Shirakawa)
   一二三 → "123" / "один-два-три" (should be Hifumi)
   小波 → "small wave" (should be Konami)

3. CHINESE READING: Chinese pinyin used instead of Japanese reading.
   高嶺 → "Гао Лин" (should be Takamine)
   白河 → "Байхэ" (should be Shirakawa)

4. COMPLETE NONSENSE: Translation has no relation to original Japanese.
   JP: "天女と結ばれる" → RU: "богиня соединяется с рыбой" (天女=heavenly maiden, NOT fish)
   JP: "腹黒い" → RU: "у вас темная кожа" (腹黒い=cunning, NOT dark skin)

Key rule: Return N if EITHER Russian OR English translation contains ML garbage.
If in doubt, return Y. Better to keep a mediocre translation than delete a usable phrase.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate phrase translations for ML garbage using LLM"
    )
    ...
    parser.add_argument("--output", default="translation_validation_report.json", help="Output report file")
...
def validate_single_phrase(
    ...
    translation_ru = phrase.get("translation_ru", "") or ""
    translation_en = phrase.get("translation_en", "") or ""

    # Pre-filter: skip LLM call if both translations are empty
    if not translation_ru.strip() and not translation_en.strip():
        return PhraseValidationResult(
            id=phrase["id"],
            text=phrase["text"],
            translation_ru=translation_ru,
            translation_en=translation_en,
            llm_response="Pre-filter: both translations empty",
            is_valid=False,
        )
...
def main():
    ...
    print(f"Translation validation ml-garbage-v1")
