...
async fn handle_normal_mode(
    ...
    let remaining_cards = start_study_usecase
        .execute(user_id, new_cards_force, false)
        .await?;

    if !remaining_cards.is_empty() {
        render_cards_grid(&remaining_cards, furigana_force)?;
    }
...
const MIN_CARD_HEIGHT: u16 = 15;
const MIN_GRID_HEIGHT: u16 = 10;
const MAX_GRID_HEIGHT: u16 = 50;

fn render_cards_grid(cards: &[StudySessionItem], furigana_shown: bool) -> Result<(), JeersError> {
    let total_cards = cards.len();
    let rows = calculate_rows(total_cards);
    let estimated_height = calculate_grid_height(rows);

    render_once(
        |frame| {
            let area = frame.area();
            let (title_area, grid_area) = split_title_and_grid(area);
            render_grid_title(title_area, total_cards, frame);
            render_grid_rows(grid_area, cards, rows, furigana_shown, frame);
        },
        estimated_height,
    )
    .map_err(|e| JeersError::RepositoryError {
...
fn calculate_rows(total_cards: usize) -> usize {
    total_cards.div_ceil(CARDS_PER_ROW)
}

fn calculate_grid_height(rows: usize) -> u16 {
    (rows as u16 * MIN_CARD_HEIGHT)
        .max(MIN_GRID_HEIGHT)
        .min(MAX_GRID_HEIGHT)
}

fn split_title_and_grid(area: Rect) -> (Rect, Rect) {
    let layout = Layout::vertical([Constraint::Length(1), Constraint::Min(0)]).split(area);
    (layout[0], layout[1])
}

fn render_grid_title(area: Rect, total_cards: usize, frame: &mut Frame) {
    let title = Line::from(
        format!("Карточки для закрепления ({}):", total_cards)
            .bold()
            .fg(Color::Cyan),
    );
...
fn render_grid_rows(
    ...
    let row_constraints: Vec<Constraint> = (0..rows)
        .map(|_| Constraint::Length(MIN_CARD_HEIGHT))
        .collect();
    let rows_layout = Layout::vertical(row_constraints).split(grid_area);

    for (row_idx, row_area) in rows_layout.iter().enumerate() {
        render_grid_row(*row_area, cards, row_idx, furigana_shown, frame);
    }
}
...
fn render_grid_row(
    ...
    let end_idx = (start_idx + CARDS_PER_ROW).min(cards.len());
    ...
    let col_constraints: Vec<Constraint> = (0..cards_in_row)
        .map(|_| Constraint::Percentage((100 / cards_in_row) as u16))
        .collect();
    let cols_layout = Layout::horizontal(col_constraints).split(row_area);

    for (col_idx, card_area) in cols_layout.iter().enumerate() {
        let card_idx = start_idx + col_idx;
        if card_idx < cards.len() {
            render_card_in_grid(&cards[card_idx], *card_area, frame, furigana_shown);
        }
    }
}

const MAX_EXAMPLES_IN_GRID: usize = 2;
const BORDER_HEIGHT: u16 = 2;

fn render_card_in_grid(
    card: &StudySessionItem,
    area: Rect,
    frame: &mut Frame,
    furigana_shown: bool,
) {
    let card_block = create_card_block();
    let mut lines = build_card_grid_lines(card, furigana_shown);
    truncate_lines_if_needed(&mut lines, area.height);
...
fn create_card_block() -> Block<'static> {
    Block::bordered()
        .border_set(border::ROUNDED)
        .border_style(Style::default().fg(Color::Green))
}

fn build_card_grid_lines(card: &StudySessionItem, furigana_shown: bool) -> Vec<Line<'_>> {
    let mut lines = vec![];
    lines.push(render_card_question_line(card, furigana_shown));
    lines.push(Line::from(""));
    lines.push(render_card_answer_line(card, furigana_shown));
    lines.push(Line::from(""));
    lines.extend(build_card_example_lines(card, furigana_shown));
    lines
}

fn render_card_question_line(card: &StudySessionItem, furigana_shown: bool) -> Line<'_> {
    if furigana_shown {
        let question_text = card.question().as_furigana();
        if question_text != card.question() {
            return furigana_renderer::render_furigana(&question_text);
        }
    }
    Line::from(card.question().bold().fg(Color::Magenta))
}

fn render_card_answer_line(card: &StudySessionItem, furigana_shown: bool) -> Line<'_> {
    if furigana_shown {
        let answer_text = card.answer().as_furigana();
        if answer_text != card.answer() {
            return furigana_renderer::render_furigana(&answer_text);
        }
    }
    Line::from(card.answer().bold().fg(Color::Magenta))
}

fn build_card_example_lines(card: &StudySessionItem, furigana_shown: bool) -> Vec<Line<'_>> {
    let mut lines = vec![];
    let examples = card.example_phrases();

    if examples.is_empty() {
        return lines;
    }

    lines.push(Line::from("Примеры:".fg(Color::Yellow).bold()));
    for example in examples.iter().take(MAX_EXAMPLES_IN_GRID) {
        lines.push(render_card_example_text_line(
            example.text(),
            furigana_shown,
        ));
        lines.push(Line::from(
...
fn render_card_example_text_line(example_text: &str, furigana_shown: bool) -> Line<'_> {
    if furigana_shown {
        let furigana_text = example_text.as_furigana();
        if furigana_text != example_text {
            let line = furigana_renderer::render_furigana(&furigana_text);
