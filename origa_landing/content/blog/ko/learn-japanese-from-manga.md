...
meta_description: "일본어로 만화를 읽는 것은 동기부여가 됩니다. 만화로부터 배우는 것은 다른 기술입니다. 실제로 어휘를 쌓는 워크플로에 대한 실전 가이드입니다: 도구, 마찰, 건너뛸 것."
...
좋은 만화 리더 *이자* 좋은 SRS *이자* 좋은 사전인 단일 앱은 없습니다. 대부분의 설정은 두세 개 도구를 엮습니다.

- **만화용 OCR 도구.** 참조 오픈소스 스택은 `manga-ocr`(다중 줄 말풍선을 포함해 만화 텍스트에 특별히 학습된 모델)을 Poricom이나 더 유연한 YomiNinja 같은 데스크톱 앱으로 감싼 것입니다. YomiNinja는 인식된 텍스트를 Yomitan으로 바로 보낼 수 있습니다. 모바일에서 KanjiSnap(iOS)은 Apple의 Live Text를 사용해 비슷한 효과를 냅니다. 빠른 조회에 편리합니다. 함정은 스타일화된 만화 글꼴에서 인식이 완벽하지 않고, 조회는 보통 일회성 사전 항목이지 SRS 카드가 아니라는 것입니다. 단어를 어딘가로 보내야 유지됩니다.
...
- **Origa** (이 워크플로의 SRS 도구). Origa는 만화 리더가 아닙니다. 이 워크플로의 SRS 측면입니다. 들어맞는 곳: 폰 카메라로 패널을 스캔하거나 스크린샷을 붙여 넣으면, OCR이 단어를 추출하고 각 단어가 문장, 읽기, 번역, 오디오와 함께 카드가 됩니다. 스스로 스케줄을 잡는 덱으로. 후리가나는 이미 배운 한자에서 숨겨지므로 읽기와 회상이 함께 훈련됩니다. ([Origa가 OCR, 후리가나, 어휘 연결을 어떻게 다루는지 보기](/ko/docs/capture); 이 단계를 담당하는 다른 도구들은 [일본어 OCR 앱 비교](/ko/blog/japanese-ocr-app)에 있습니다.)
...
아닙니다. N5-N4 읽기 레벨의 후리가나가 많은 만화(소년/소녀)로 시작하세요. 리스트보다 문맥에서 한자를 더 빨리 배웁니다. 함정은 읽은 것을 유지하려면 SRS가 필요하다는 것입니다. 그렇지 않으면 일주일 안에 한자를 잊습니다.
