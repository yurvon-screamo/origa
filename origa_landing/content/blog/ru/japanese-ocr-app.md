...
Через ту же проблему идёт другим путём **mokuro**: вместо распознавания кадров по одному он заранее обрабатывает целые тома манги в HTML-страницы, где текст становится выделяемым и наводимым. Результат читают в браузере, а всплывающий словарь делает Yomitan. Это пакетный конвейер, а не приложение — возни больше, зато для майнинга это сильный путь. Противоположный подход у **Yomu**: iOS-приложение для чтения, где OCR и словарь встроены сразу, — чтобы читать мангу и бумажные страницы без сборки инструментария.
...
### OCR, встроенный в обучение (где стоит Origa)
...
## Инструменты одним взглядом

Вопрос чеклиста, который разделяет их все, — «куда попадает результат OCR». Собрано в одну таблицу:

| Инструмент | Категория | Куда попадает результат OCR |
| --- | --- | --- |
| Google Lens | Универсальный | Перевод поверх фото — прочитали раз, ничего не сохранилось |
| Apple Live Text | Универсальный | Выделяемый текст — сами несёте его в другое приложение |
| Imiwa, Nihongo | Словарь + OCR | Словарная статья; в лучшем случае список избранных слов |
| KanjiSnap | Словарь + OCR | Словарная статья (распознавание через Live Text) |
| manga-ocr (модель) | Для манги | Распознанный текст — для поиска и майнинговых конвейеров |
| Poricom, YomiNinja | Для манги (десктоп) | Текст кадра → поиск по ходу чтения |
| Namida-OCR | Для манги (браузер) | Текст кадра → поиск в браузере |
| mokuro | Предобработка манги | Целые тома как страницы с выделяемым текстом; чтение с Yomitan |
| Yomu | Читалка + OCR | Чтение манги/фото со встроенным поиском слов |
| Origa | Обучение: OCR → карточки | OCR → словарь → карточки с повторениями по расписанию FSRS |

Первые девять строк выдают текст или поиск. Последняя — единственная, где сканирование по умолчанию кормит конвейер повторений.
...
- **Работает локально или в облаке?** Облачный OCR часто точнее, но требует интернета и отправляет ваши фото на сервер. Локальный OCR (NDLOCR, Apple Vision framework, on-device ML-модели) работает офлайн и приватен. Универсально лучшего нет — выбирайте по тому, что важнее: офлайн и приватность или максимальная точность.
...
### Работает ли OCR офлайн?

Зависит от приложения. Облачный OCR (большинство переводчиков) требует интернета. Локальный OCR (NDLOCR-Lite в Origa, Apple Live Text) работает офлайн. Если занимаетесь в поезде или самолёте — локальность важна.
