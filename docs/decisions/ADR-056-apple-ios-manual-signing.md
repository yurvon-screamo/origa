...
- **Известная хрупкость**: инъекция `PROVISIONING_PROFILE_SPECIFIER` строит
  контракт на строковый формат pbxproj, генерируемый xcodegen (в CI ставится
  плавающей версией через brew). При дрейфе формата инъекция падает громко
  (count-проверка) — релизный контур станет красным без изменений в репо.
  Лечение на такой случай: сверить формат pbxproj с ожидаемым
  (`CODE_SIGN_ENTITLEMENTS = "..."` в обеих конфигурациях таргета) и
  адаптировать sed, либо запинить версию xcodegen в CI.
