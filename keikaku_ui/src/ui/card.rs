...
pub fn Card(class: Option<String>, delay: Option<String>, children: Element) -> Element {
    ...
    rsx! {
        div { class: "bg-surface rounded-[2rem] p-8 shadow-soft hover:shadow-soft-hover transition-shadow duration-500 animate-enter {delay_class} relative overflow-hidden {class_str}",
            {children}
        }
    }
}
