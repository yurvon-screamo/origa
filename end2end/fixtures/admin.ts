...
export async function createTestUser(
    ...
    const response = await fetchWithTimeout(
        `${getTrailBaseUrl()}/api/_admin/user`,
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
    ...
    if (!response.ok) {
        ...
        if (response.status === 409 || text.includes("already exists")) {
            throw new Error(
                `User already exists: ${email}. Cannot retrieve UUID without creation.`,
            );
        }
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            if (attempt < retries && isRetryable) {
                console.error(
                    `[admin] Retrying delete user ${userEmail} (${userId}), attempt ${attempt + 1}`,
                );
...
export async function listAllTestUsers(
    ...
    while (offset < totalRowcount) {
        const response = await fetchWithTimeout(
            `${getTrailBaseUrl()}/api/_admin/user?limit=${PAGE_SIZE}&offset=${offset}`,
            {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${token}`,
        ...
        const e2eUsers = data.users.filter(
            (u) =>
                u.email.startsWith("e2e-") && u.email.endsWith("@origa.local"),
        );
        allE2eUsers.push(
            ...e2eUsers.map((u) => ({ id: u.id, email: u.email })),
        );
