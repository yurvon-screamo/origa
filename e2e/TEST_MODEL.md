...
- `KanjiPage` - `e2e/pages/KanjiPage.ts`

### Test Cases

|| ID       | Test Case                                        | Precondition                   | Steps                                                         | Expected Result                                                          |
| -------- | ------------------------------------------------ | ------------------------------ | ------------------------------------------------------------- | ------------------------------------------------------------------------ |
| KANJI-01 | Отображение всех элементов страницы кандзи       | Пользователь авторизован       | 1. Перейти на `/kanji`                                        | Отображаются: заголовок "Кандзи", поле поиска, кнопка "+", кнопка "Назад" |
| KANJI-02 | Отображение фильтров категорий                   | Пользователь на странице кандзи | 1. Проверить наличие фильтров                                 | Отображаются фильтры: Все, Новые, Сложные, В процессе, Изученные         |
| KANJI-03 | Отображение списка карточек кандзи               | Пользователь на странице кандзи | 1. Проверить список карточек                                  | Отображается минимум одна карточка с кандзи                               |
| KANJI-04 | Поиск по кандзи                                  | Пользователь на странице кандзи | 1. Ввести кандзи в поле поиска                                | Отображаются карточки с匹配ающими кандзи                                  |
| KANJI-05 | Поиск по переводу (описанию)                     | Пользователь на странице кандзи | 1. Ввести перевод в поле поиска                               | Отображаются карточки с совпадающими описаниями                           |
| KANJI-06 | Пустое состояние при отсутствии результатов      | Пользователь на странице кандзи | 1. Ввести несуществующий кандзи                               | Отображается сообщение "Кандзи не найдено"                                |
| KANJI-07 | Переключение фильтров                            | Пользователь на странице кандзи | 1. Нажать фильтр "Новые"<br>2. Нажать фильтр "Все"            | Фильтры переключаются, активный фильтр подсвечивается                     |
| KANJI-08 | Отображение счетчиков в фильтрах                 | Пользователь на странице кандзи | 1. Проверить счетчик в фильтре "Все"                          | Счетчик показывает количество >= 0                                        |
| KANJI-09 | Возврат на главную страницу                      | Пользователь на странице кандзи | 1. Нажать кнопку "Назад"                                      | Редирект на `/home`                                                       |
| KANJI-10 | Открытие модального окна добавления кандзи       | Пользователь на странице кандзи | 1. Нажать кнопку "+"                                          | Открывается модальное окно "Добавить кандзи"                              |
| KANJI-11 | Закрытие модального окна по кнопке "Отмена"      | Модальное окно открыто         | 1. Нажать кнопку "Отмена"                                     | Модальное окно закрывается                                                |
| KANJI-12 | Отображение кнопок выбора уровня JLPT            | Модальное окно открыто         | 1. Проверить наличие кнопок                                   | Отображаются кнопки: N5, N4, N3, N2, N1                                  |
| KANJI-13 | Переключение уровня JLPT                         | Модальное окно открыто         | 1. Нажать кнопку "N4"                                         | Кнопка "N4" подсвечивается как активная                                   |
...
- [x] Поле поиска
- [x] Карточки кандзи (кандзи, описание, радикалы, примеры, метрики)
- [x] Сообщение о пустом результате

#### Modal

- [x] Заголовок "Добавить кандзи"
- [x] Селектор уровня JLPT (N5, N4, N3, N2, N1)
- [x] Список доступных кандзи
- [x] Счетчик выбранных кандзи
- [x] Кнопка "Добавить"
- [x] Кнопка "Отмена"

---

## Module 6: Lesson Page

### Page Objects

- `LessonPage` - `e2e/pages/LessonPage.ts`

### Test Cases

| ID        | Test Case                                   | Precondition                   | Steps                                                     | Expected Result                                              |
| --------- | ------------------------------------------- | ------------------------------ | --------------------------------------------------------- | ------------------------------------------------------------ |
| LESSON-01 | Отображение заголовка страницы урока        | Пользователь авторизован       | 1. Перейти на `/lesson`                                   | Отображается заголовок "Урок"                                |
| LESSON-02 | Отображение прогресс-бара                  | Пользователь на странице урока | 1. Проверить наличие прогресс-бара                        | Отображается прогресс-бар с текущим прогрессом               |
| LESSON-03 | Отображение карточки с вопросом            | Пользователь на странице урока | 1. Проверить содержимое карточки                           | Отображаются: тип карточки (тег), вопрос, кнопка "Показать ответ" |
| LESSON-04 | Показ ответа при нажатии кнопки            | Пользователь на странице урока | 1. Нажать "Показать ответ"                                 | Отображается ответ и 4 кнопки оценки                         |
| LESSON-05 | Отображение кнопок оценки                  | После показа ответа            | 1. Проверить наличие кнопок                                | Отображаются кнопки: "Не знаю", "Плохо", "Знаю", "Идеально"  |
| LESSON-06 | Возврат на главную при нажатии кнопки назад | Пользователь на странице урока | 1. Нажать "Назад"                                          | Редирект на `/home`                                          |
| LESSON-07 | Экран завершения урока                     | Пройдены все карточки          | 1. Оценить последнюю карточку                              | Отображается "Урок завершён!" с результатами                 |
...
- [x] Прогресс-бар (текущий/всего)
- [x] Тег типа карточки (Слово/Кандзи/Грамматика)
- [x] Вопрос (текст карточки)
- [x] Кнопка "Показать ответ"
- [x] Ответ (после показа)
- [x] Радикалы (для кандзи)

#### Rating Buttons

- [x] Кнопка "Не знаю" (Again)
- [x] Кнопка "Плохо" (Hard)
- [x] Кнопка "Знаю" (Good)
- [x] Кнопка "Идеально" (Easy)

#### Complete Screen

- [x] Заголовок "Урок завершён!"
- [x] Счётчик новых карточек
- [x] Счётчик повторённых карточек
- [x] Кнопка "На главную"
